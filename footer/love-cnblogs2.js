...
function allpage(c) {
    var custom = "";
    for (var i = 0; i < c.custom.length; i++) { if (c.custom[i].istarget == true) { custom = custom + '<li><a href="' + c.custom[i].link + '" target="_black">' + c.custom[i].name + "</a></li>" } else { custom = custom + '<li><a href="' + c.custom[i].link + '">' + c.custom[i].name + "</a></li>" } }
    $("#navList").html(custom);
    if (mobile_flag) {} else {
    ...
    tippy(".skin_btn", { content: skin, theme: "tomato", allowHTML: true, animation: "scale", duration: 500, arrow: true, hideOnClick: "false", interactive: true });
    var le = $("link").length;
    for (var i = 0; i < le; i++) { var rel = $("link").eq(i).attr("rel"); if (rel == "shortcut icon") { $("link").eq(i).attr("href", c.logoimg) } }
    var details = [{ field: "name", literal: "昵称", }, { field: "introduction", literal: "标语", }, { field: "url", literal: "链接地址", }, { field: "headurl", literal: "头像地址", }];
    var newarray = shuffle(c.unionbox);
    var listfriends = "";
    for (var i = 0; i < newarray.length; i++) { listfriends = listfriends + '<div class="unionbox"><div class="info"><div class="headimg"><img src="' + newarray[i].headurl + '" alt=""></div><div class="content"><h3 class="title">' + newarray[i].name + '</h3><p class="desc">' + newarray[i].introduction + '</p><a class="btn" target="_black" href="' + newarray[i].url + '">直达</a></div></div></div>' }
    $(".unionbox-box").html(listfriends);
    var detailsfriends = "";
    for (var i = 0; i < details.length; i++) { detailsfriends = detailsfriends + "<tr><td>" + details[i].field + "</td><td>" + details[i].literal + "</td></tr>" }
    $(".friendstable").append(detailsfriends);
    if (c.isGratuity != false) { tippy(".gratuity", { content: c.gratuity, theme: "tomato", allowHTML: true, animation: "scale", duration: 500, arrow: true, hideOnClick: "false", interactive: true }) } else { $(".gratuity").hide() }
}
...
var load_face = function(a) {
    var OwO_demo = new OwO({ logo: "OωO表情", container: document.getElementsByClassName("OwO")[0], target: document.getElementById("tbCommentBody"), api: "https://miluluyo.github.io/OwO.json", position: "down", width: "70%", maxHeight: "250px" });
    a.classList.add("OwO-open");
    a.onclick = null
};
$("#commentbox_opt").nextAll().remove();
$("#btn_comment_submit").val("提交评论 (Ctrl + Enter)");
$(document).on("click", "#btn_edit_comment", function() { $(".OwO").show() });
$(document).on("click", "#btn_preview_comment", function() { $(".OwO").hide() });
$(window).resize(function() { var w = document.body.clientWidth; if (w > 1300) {} else { if (960 < w) { if ($("#mainContent").css("width") != "90%") { $("#mainContent").css("width", "90%") } } else { if (0 < w) { if ($("#mainContent").css("width") != "90%") { $("#mainContent").css("width", "90%") } } } } });
var oInfo = $(".blogTitle");
...
$(".catalogue_btn").click(function() {
    ...
    if ($("#catalogue").css("visibility") == "hidden") {
        ...
        setTimeout(function() { $("#catalogue").css("display", "none") }, 500)
    }
    if (w <= 1300) { $("#mainContent").css("width", "90%") } else {
...
function GenerateContentList() {
    ...
    if (jquery_h1_list.length == 0) { $(".catalogueMain").prepend('<p style="font-size:18px;text-align: center;padding-bottom: 10px;"><b>阅读目录(Content)</b></p><p>此页目录为空</p>'); return }
    ...
    for (var i = 0; i < jquery_h1_list.length; i++) {
        ...
        if (jquery_h2_list.length > 0) { li_content += '<ul class="second_class_ul">' }
        for (var j = 0; j < jquery_h2_list.length; j++) {
            ...
            var nextH2Index = j + 1;
            var next;
            if (nextH2Index == jquery_h2_list.length) { if (i + 1 == jquery_h1_list.length) { next = jquery_h1_list[0] } else { next = jquery_h1_list[i + 1] } } else { next = jquery_h2_list[nextH2Index] }
            var jquery_h3_list = $(jquery_h2_list[j]).nextUntil(next, "h3");
            if (jquery_h3_list.length > 0) { li_content += '<ul class="third_class_ul">' }
            ...
            if (jquery_h3_list.length > 0) { li_content += "</ul>" }
            li_content += "</li>"
        }
        if (jquery_h2_list.length > 0) { li_content += "</ul>" }
        li_content += "</li>";
        content += li_content
    }
    content += "</ul>";
...
if (catalogue == null) {} else {
    if (catalogue == "show") {
        ...
        if (catalogue == "none") {
            ...
            setTimeout(function() { $("#sideBar").css("display", "none") }, 500)
        }
    }
}
...
$(document).on("click", ".fly_top", function() { $("body,html").animate({ scrollTop: 0 }, 600) });
...
$(".artice_recommend").click(function() { $(".artice_recommend").attr("onclick", "") });
$(".catalogue").click(function() {
    var w = $(window).width();
    if ($("#sideBar").css("visibility") == "hidden") {
        $("#sideBar").css("display", "inline-block");
        if (w > 1300) { $("#sideBar").attr("style", "visibility: visible; clip-path: circle(100% at 50% 50%); transition: all 0.3s ease-in-out 0s;") } else { $("#sideBar").attr("style", "visibility: visible; clip-path: circle(100% at 50% 50%); transition: all 0.3s ease-in-out 0s;") }
    ...
    if (w <= 1300) { $("#mainContent").css("width", "90%") } else { $("#mainContent").css("width", "calc(100% - 250px)") }
    return false
});
